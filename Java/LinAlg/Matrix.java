...
public class Matrix {
     ...
     */
    public void print() {
        String m = Arrays.deepToString(this.matrix).replace("], ", "]\n") // Each row takes a line
     ...
     * 
     * @return A tuple [m, n] containing the size of the matrix, where m is the
     *         number of rows and n in the number of columns.
     */
    public int[] size() {
    ...
    public Matrix cloneMatrix() {
        ...
        for (int i = 0; i < rowCount; i++)
            M[i] = this.matrix[i].clone();
        return new Matrix(M);
    }

    /**
     * Converts the matrix into Row-Reduced Echelon Form (RREF). This algorithm is a
     * translation from the pseudo-code found at wikipedia.org
     * (https://en.wikipedia.org/wiki/Gaussian_elimination#Pseudocode) into Java
     * code.
     * 
    ...
    public Matrix RREF() {
        ...
        while (h < M.rowCount && k < M.columnCount) {
            ...
            for (int i = h + 1; i < M.rowCount; i++) {
                if (Math.abs(M.getElement(i, k)) > Math.abs(M.getElement(i_max, k)))
                    i_max = i;
            }
     ...
     */
    public Matrix elSubtract(double n) {
        return elAdd(-1 * n);
    }
     ...
     */
    public Matrix elDivide(double n) {
        return elMultiply(1.0 / n);
    }
    ...
    public Vect getRow(int r) {
        return new Vect(this.matrix[r].clone());
    }
    ...
    public Vect getColumn(int c) {
        ...
        for (int i = 0; i < rowCount; i++)
            column[i] = getElement(i, c);
        return new Vect(column);
    }

    /**
     * Performs the augmentation (A.K.A. concatenation) of this matrix and the
     * matrix M.
     * 
     * @param M The matrix to be concatenated to this matrix.
     * @return A new Matrix whose elements are the augmentation of this matrix and
     *         the matrix M.
     */
    public Matrix augment(Matrix M) {
        if (this.rowCount != M.rowCount)
            throw new Error("Matrices must contain the same number of rows.");
    ...
    public Matrix inverse() {
        if (!isSquare)
            throw new Error("Matrix inverse cannot be computed because the matrix is not square.");

        double determinant = det();
        if (determinant == 0)
            throw new Error("Matrix inverse cannot be computed because matrix is singular.");
    ...
    public double det() {
        if (!isSquare)
            throw new Error("Matrix determinant cannot be computed because the matrix is not square.");
    ...
    /**
     * Computes the sub-matrix of this matrix when the row r and the column c are
     * ignored from this matrix.
     * 
     * @param r The row to ignore.
     * @param c The column to ignore.
     * @return A new Matrix whose elements are the elements of the sub-matrix of
     *         this matrix when the row r and the column c are ignored from this
     *         matrix.
     */
     ...
     * 
     * @return A new Matrix whose elements are the elements of the matrix of minors
     *         for this matrix.
     */
     ...
     * 
     * @return A new Matrix whose elements are the elements of the matrix of
     *         cofactors for this matrix.
     */
    ...
    /**
     * Returns the adjugate of this matrix. As defined by wikipedia, the adjugate,
     * classical adjoint, or adjunct of a square matrix is the transpose of its
     * cofactor matrix
     * 
     * @return A new Matrix whose elements are the elements of the adjugate of this
     *         matrix.
     */
    public Matrix adjugate() {
        return cofactorsMatrix().T();
    }

    /**
     * Maps each element of this matrix to a new Matrix based on the provided
     * mapping function f.
     * 
     * @param f The mapping function to be applied to each element of this matrix.
     * @return A new Matrix whole elements are the mapped elements of this matrix
    ...
    public static Matrix multiply(Matrix... matrices) {
        if (matrices.length <= 1)
            throw new Error("Matrix multiplication can only happen between 2 or more matrices.");

        Matrix M = multiply(matrices[0], matrices[1]);
        for (int i = 0; i < matrices.length - 2; i++)
            M = multiply(M, matrices[i + 2]);
        return M;
    }

    /**
     * Generates an <code>mxn</code> matrix with random elements between zero and
     * one.
     * 
    ...
    public static Matrix rand(int m, int n) {
        ...
        for (int i = 0; i < m; i++)
            for (int j = 0; j < n; j++)
                matrixEls[i][j] = random.nextDouble();
    ...
    /**
     * Computes the sparsity of the matrix. As defined by Wikipedia.org, a sparse
     * matrix or sparse array is a matrix in which most of the elements are zero.
     * 
